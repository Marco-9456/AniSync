 ...
 *
 * [com.anisync.android.data.util.safeApiCall] is a free function called from 92 repository sites,
 * so it has nowhere to inject a `Context` from. The singleton publishes itself into [instance] when
...
class ApiErrorMessages @Inject constructor(
    ...
    fun message(error: ApiError): String = when (error) {
